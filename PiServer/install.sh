...
auto_install() {
    ...
    # NAS-instellingen
    # Share-naam ligt vast op "Opslag" - niet meer aanpasbaar. Dit was
    # eerder vrij intypbaar, wat samen met hardcoded Y:/Z:-verwachtingen
    # elders in de suite tot problemen leidde zodra iemand een andere
    # naam koos. Zie ook Gedeeld/pinas_schijven.py, dat op deze vaste
    # naam zoekt in plaats van op een vaste schijfletter.
    SHARE="Opslag"
...
[Service]
Type=oneshot
ExecStart=/bin/bash -c 'for d in /mnt/opslag /mnt/backup; do [ -d "\$d" ] && chown pi:pi "\$d" && chmod 775 "\$d"; done; for n in /mnt/opslag/nextcloud-data /mnt/backup/nextcloud-data; do [ -d "\$n" ] && chown -R www-data:www-data "\$n" && chmod -R 755 "\$n"; done'
RemainAfterExit=yes
[Install]
