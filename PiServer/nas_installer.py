...
def CFrame(parent, **kw):
    if CTK:
        fg = kw.pop('bg', BG)
        kw.pop('relief', None)
        return ctk.CTkFrame(parent, fg_color=fg, corner_radius=8, **kw)
    return tk.Frame(parent, **kw)

def CButton(parent, text, command, bg=None, fg=None, **kw):
    if CTK:
        for k in ['relief','padx','pady','activebackground','activeforeground',
                  'borderwidth','cursor','highlightthickness','highlightbackground',
                  'highlightcolor','state','width','anchor']:
            kw.pop(k, None)
        font = kw.pop('font', ("Segoe UI", 10))
        return ctk.CTkButton(parent, text=text, command=command,
                            fg_color=bg or PANEL, text_color=fg or FG,
                            hover_color=PANEL2, corner_radius=8,
                            border_width=1, border_color=fg or FG,
                            font=font, height=32)
    return tk.Button(parent, text=text, command=command,
                    bg=bg or PANEL, fg=fg or FG,
                    relief="flat", cursor="hand2",
                    borderwidth=1, highlightthickness=1,
                    highlightbackground=fg or FG, **kw)
...
ACCENT = ACCENT_PINAS   # blauw — NAS Installer

# ── Helpers ───────────────────────────────────────────────────────────────────

def is_pi5():
    """Detecteer of we op een Pi 5 draaien."""
    model = sh("cat /proc/device-tree/model 2>/dev/null")
    return "Pi 5" in model or "Raspberry Pi 5" in model

def get_vnc_port():
    return "5901" if is_pi5() else "5900"
...
def setup_nopasswd():
    """Stelt NOPASSWD in voor sudo voor de pi-gebruiker, in een los
    sudoers.d-bestand (veiliger dan het hoofdbestand bewerken). Dit is
    vereist voor de externe SSH-installatieknoppen in Pi NAS Menu, die
    anders blijven hangen op een wachtwoordprompt die via een
    niet-interactieve SSH-sessie nooit beantwoord kan worden. Veilig om
    herhaaldelijk te draaien -- overschrijft altijd met dezelfde inhoud."""
    try:
        user = sh("logname 2>/dev/null || echo pi") or "pi"
        regel = f"{user} ALL=(ALL) NOPASSWD: ALL"
        with open("/tmp/pi-nopasswd", "w") as f:
            f.write(regel + "\n")
        subprocess.run(
            "sudo cp /tmp/pi-nopasswd /etc/sudoers.d/pi-nopasswd && "
            "sudo chmod 440 /etc/sudoers.d/pi-nopasswd && "
            "sudo visudo -c -f /etc/sudoers.d/pi-nopasswd",
...
def update_rechten_service():
    ...
    # Wacht op de ECHTE mountpoints (max 60 sec), niet alleen op het
    # generieke local-fs.target -- externe schijven via smart plug
    # kunnen pas seconden na boot daadwerkelijk gemount zijn, waardoor
    # deze service voorheen permanent op 'failed' kon blijven staan
    # voor iets dat bij een volgende boot-poging vanzelf zou lukken.
    wacht_cmd = " && ".join(
        f'for i in $(seq 1 60); do mountpoint -q "{mp}" && break; sleep 1; done'
        for mp in mounts
    )

    cmd=f"chown {user}:{user} {paths} && chmod 775 {paths}"
    for mp in mounts:
...
[Service]
Type=oneshot
ExecStart=/bin/bash -c '{wacht_cmd}; {cmd}'
RemainAfterExit=yes
Restart=on-failure
RestartSec=10
...
# ════════════════════════════════════════════════════════════════════════════
class NASInstaller(ctk.CTk if CTK else tk.Tk):
    PAGES=["Welkom","Netwerk","SSH","Systeem bijwerken","Schijf beheer",
           ...
           "💾 Configuratie","⬆ Scripts bijwerken","🔬 Diagnose","📋 Systeem info","❓ Help","🛠 Beheer","⚙️ Cockpit","🐳 Docker"]

    def __init__(self):
        super().__init__()
        self.title("🍓 Raspberry Pi NAS Installer")
        self.geometry("1020x810"); self.minsize(920,700); self.configure(fg_color=BG) if CTK else self.configure(bg=BG)
        # Vars
        ...
        # (Nextcloud-velden verwijderd - Nextcloud installeert nu via het
        # eigen pinas_nextcloud.sh, niet meer via deze GUI)
    ...
    def _build_chrome(self):
        ...
        tk.Label(hdr,text="🍓  Raspberry Pi NAS Installer  v1.1.0",
        ...
        self.content=ctk.CTkScrollableFrame(self,fg_color=BG,corner_radius=0) if CTK else tk.Frame(self,bg=BG)
        ...
        self.btn_back=CButton(nav,text="← Terug",command=self._prev,
                                 bg=PANEL2,fg=FG,relief="flat",padx=12,pady=5,
                                 font=("Segoe UI",11),cursor="hand2")
        self.btn_back.pack(side="left",padx=12)
        self.btn_skip=CButton(nav,text="⏭ Overslaan",command=self._skip,
                                 bg=PANEL2,fg=YELLOW,relief="flat",padx=10,pady=5,
                                 font=("Segoe UI",11),cursor="hand2")
        self.btn_skip.pack(side="left",padx=4)
        self.btn_next=CButton(nav,text="Volgende →",command=self._next,
    ...
    def _show_page(self,idx):
        ...
        self.btn_back.configure(state="normal" if self._history or idx>0 else "disabled", command=self._prev, text_color=FG)

        if idx >= 9:
            # Extra pagina's — alleen Terug knop
            self.btn_skip.configure(state="disabled", text_color="#444")
            self.btn_next.configure(state="disabled", text="", text_color=PANEL2)
        elif idx==8:
            self.btn_skip.configure(state="disabled", text_color="#444")
            self.btn_next.configure(text="Sluiten", state="normal", command=self.destroy, text_color=BG)
        else:
            skip_hidden=idx in (0,7)
            self.btn_skip.configure(state="disabled" if skip_hidden else "normal",
                                  text_color="#444" if skip_hidden else YELLOW)
            if idx==7:
                self.btn_next.configure(text="Volgende →", state="disabled", command=self._next, text_color=BG)
            else:
                self.btn_next.configure(text="Volgende →", state="normal", command=self._next, text_color=BG)
    ...
    def _open_browser(self, url):
        ...
        e.focus_set()
        self._btn(popup,"✖  Sluiten",popup.destroy,bg=PANEL2,fg=FG).pack(pady=(0,12))
    ...
    def _btn(self,parent,text,cmd,bg=PANEL2,fg=FG,bold=False):
        if CTK:
            hover=self._darker(bg)
        ...
        return tk.Button(parent,text=text,command=cmd,bg=bg,fg=fg,relief="flat",
                         padx=10,pady=5,font=("Segoe UI",10),cursor="hand2")

    def _darker(self,color):
        """Maak een kleur iets donkerder voor hover effect."""
        try:
            r=int(color[1:3],16); g=int(color[3:5],16); b=int(color[5:7],16)
            r=max(0,r-30); g=max(0,g-30); b=max(0,b-30)
            return f"#{r:02x}{g:02x}{b:02x}"
        except: return PANEL2
    ...
    def _pg_welcome(self):
        ...
        self._btn(p_logout,"🔒  Uitloggen",self._uitloggen,bg=PANEL2,fg=YELLOW).pack(side="right")
        ...
        for txt,idx,bg,fg in [
            ("💾  Schijf beheer", 4, "#1d4ed8", FG),
            ("⚙️  NAS software",  5, "#4338ca", FG),
            ("🌐  FileBrowser",   10,"#6d28d9", FG),
            ("⚙️  Cockpit",      20, "#b45309", FG),
            ("🖥️  Desktop & VNC",11, "#065f46", FG)]:
            if CTK:
                b=ctk.CTkButton(br,text=txt,command=lambda i=idx:self._jump(i),
                               fg_color=bg,text_color=fg,
                               hover_color=self._darker(bg) if hasattr(self,"_darker") else PANEL2,
                               corner_radius=8,border_width=0,
                               font=("Segoe UI",11),height=36)
            else:
                b=tk.Button(br,text=txt,command=lambda i=idx:self._jump(i),
                           bg=bg,fg=fg,relief="flat",padx=12,pady=7,cursor="hand2")
            b.pack(side="left",padx=(0,6))
        tk.Label(p2,text="",bg=PANEL,height=1).pack(anchor="w")
        br2=tk.Frame(p2,bg=PANEL); br2.pack(anchor="w",pady=(0,2))
        for txt,idx,bg,fg in [
            ("🔑  SSH-sleutel",       12, PANEL2, FG),
            ("💾  Configuratie",      14, PANEL2, FG),
            ("⬆  Scripts bijwerken",  15, PANEL2, FG),
            ("🔬  Diagnose",          16, PANEL2, FG),
            ("📋  Systeem info",      17, "#0d4f4a", TEAL),
            ("❓  Help",              18, PANEL2, FG),
            ("🛠  Beheer",            19, "#3d2e00", YELLOW),
            ("🗑  Deïnstalleren",     13, "#3d0f0f", RED),
        ]:
            if CTK:
                b=ctk.CTkButton(br2,text=txt,command=lambda i=idx:self._jump(i),
                               fg_color=bg,text_color=fg,hover_color=PANEL2,
                               corner_radius=6,border_width=0,
                               font=("Segoe UI",10),height=30)
            else:
                b=tk.Button(br2,text=txt,command=lambda i=idx:self._jump(i),
                           bg=bg,fg=fg,relief="flat",padx=10,pady=5,cursor="hand2")
    ...
    def _pg_ssh(self):
        ...
        self._btn(br,"🔑 SSH-sleutel beheren",lambda:self._jump(12),bg=PANEL2,fg=FG).pack(side="left")
    ...
    def _do_update(self):
        self.btn_next.configure(state="disabled"); self.upd_out.delete("1.0",tk.END)
        def done():
    ...
    def _tab_overzicht(self,parent):
        ...
        self._btn(hdr,"🔄 Vernieuwen",lambda:self._ov_refresh(),bg=PANEL2,fg=FG).pack(side="right")
    ...
    def _tab_koppelen(self,parent):
        ...
        self._btn(br,"🔍 Schijven",lambda:(self.kop_out.delete("1.0",tk.END),
                  ...
                  bg=PANEL2,fg=FG).pack(side="left")
    ...
    def _do_koppelen(self):
        ...
        def after():
            def bg():
                ...
                setup_nopasswd(); update_rechten_service()
    ...
    def _tab_verwijderen(self,parent):
        ...
        self._rm_preview()
        br=tk.Frame(parent,bg=BG); br.pack(anchor="w",padx=6,pady=(4,2))
        self._btn(br,"🔄 Lijst vernieuwen",self._rm_reload,bg=PANEL2,fg=FG).pack(side="left",padx=(0,8))
    ...
    def _tab_diagnose(self,parent):
        ...
        for lbl,cmd in [("🔍 lsblk","lsblk -o NAME,SIZE,TYPE,FSTYPE,MOUNTPOINT"),
            ...
            self._btn(br,lbl,lambda c=cmd:(self.diag_out.delete("1.0",tk.END),
                      ...
                      bg=PANEL2,fg=FG).pack(side="left",padx=(0,5))
    ...
    def _pg_method(self):
        ...
        if samba_ok:
            ...
            tk.Label(p,text="⚠  Al geïnstalleerd: Samba",
    ...
    def _pg_install(self):
        ...
        self._btn(br,"← Annuleren (terug naar methode)",
                  lambda:self._show_page(5),bg=PANEL2,fg=FG).pack(side="left")
    ...
    def _do_install(self):
        m=self.method.get(); share="Opslag"  # vast, niet meer instelbaar - zie pinas_schijven.py
    ...
    def _pg_done(self):
        ...
        m=self.method.get(); ip=get_ip(); share="Opslag"  # vast, niet meer instelbaar
    ...
    def _pg_initsetup(self):
        ...
        for val,lbl in [("A","Samba + Cockpit (aanbevolen)")]:
    ...
    def _run_initsetup(self):
        ...
        self.btn_next.configure(state="disabled"); self.init_out.delete("1.0",tk.END)
        def log(t,tag=None):
        ...
        def run():
            ...
            share="Opslag"  # vast, niet meer instelbaar
    ...
    def _pg_filebrowser(self):
        ...
        self._btn(br2,"⏹  Stoppen",self._fb_stop,bg=PANEL2,fg=FG).pack(side="left",padx=(0,8))
        if fb_enabled:
            self._btn(br2,"Uitschakelen bij opstarten",
                      lambda:sh("sudo systemctl disable filebrowser") or self._jump(10),bg=PANEL2,fg=FG).pack(side="left")
        else:
        ...
        self._btn(br3,"🔑  Wachtwoord resetten",self._fb_reset,bg=PANEL2,fg=FG).pack(side="left")
...
"""
    ...
    def _pg_desktop(self):
        ...
        tk.Label(p,text=f"VNC: {'✅ geïnstalleerd op '+ip+':'+get_vnc_port() if vnc_ok else 'niet geïnstalleerd'}",
        ...
        tk.Label(p3,text="VNC — grafische omgeving via Windows/tablet",
                 font=("Segoe UI",12,"bold"),bg=PANEL,fg=ACCENT).pack(anchor="w",pady=(0,4))
        tk.Label(p3,text=f"{'Pi 5: TigerVNC Viewer' if is_pi5() else 'Pi 4: RealVNC Viewer'}: {ip}:{get_vnc_port()}\n"
        ...
        self._btn(br3,"⏹  Stoppen",self._vnc_stop,bg=PANEL2,fg=FG).pack(side="left",padx=(0,6))
    ...
    def _vnc_install(self):
        ...
        ip=get_ip()
        def done():
            self._dei_log(f"✅  VNC geïnstalleerd en actief op {ip}:{get_vnc_port()}","ok")
            self._dei_log("   Inloggen met Pi OS gebruikersnaam en wachtwoord","info")
            self._dei_log(f"   {'TigerVNC Viewer: https://github.com/TigerVNC/tigervnc/releases' if is_pi5() else 'RealVNC Viewer: https://www.realvnc.com/en/connect/download/viewer/'}","info")
        self._run_bg([
            ("apt update","sudo apt-get update -y"),
            *([
                ("X11 instellen","sudo raspi-config nonint do_wayland W2"),
                ("RealVNC verwijderen","sudo apt-get purge -y realvnc-vnc-server 2>/dev/null; true"),
                ("TigerVNC installeren","sudo apt-get install -y tigervnc-standalone-server"),
                ("xhost permanent","echo 'xhost +local:root' >> /home/pi/.bashrc"),
                ("TigerVNC starten","sudo -u pi vncserver -kill :1 2>/dev/null; true && sudo -u pi vncserver :1 -geometry 1920x1080 -depth 24 -localhost no"),
            ] if is_pi5() else [
    ...
    def _vnc_start(self):
        ...
        ip=get_ip()
        self.desk_out.insert(tk.END,f"✅  VNC gestart op {ip}:{get_vnc_port()}\n","ok")
    ...
    def _pg_sshkey(self):
        ...
        self._btn(br,"Wachtwoordlogin inschakelen",self._ssh_pw_on,bg=PANEL2,fg=FG).pack(side="left")
    ...
    def _ssh_pw_off(self):
        ...
        if os.path.exists(cloud_init):
            sh(f"sudo sed -i 's/PasswordAuthentication yes/PasswordAuthentication no/' {cloud_init}")
...
# ────────────────────────────────────────────────────────────────────────────
    def _pg_deinstall(self):
        ...
        tk.Label(p,text="Verwijderen van Nextcloud: gebruik pinas_nextcloud_verwijderen.sh",
                 font=("Segoe UI",9),bg=PANEL,fg=DIM).pack(anchor="w",pady=(4,0))
        ...
        for naam,cmd,enabled in [
            ...
            self._btn(p2,f"🗑  {naam} verwijderen",cmd,
                      bg=WARN if enabled else PANEL2,
        ...
        tk.Label(p3,text="Verwijdert alles: Samba, FileBrowser, Cockpit, Desktop.\n"
    ...
    def _dei_alles(self):
        if not messagebox.askyesno("BEVESTIGEN",
                "Alles verwijderen: Samba, FileBrowser, Cockpit, Desktop?\n"
                "(Nextcloud: gebruik apart pinas_nextcloud_verwijderen.sh)\n"
    ...
    def _pg_nasconfig(self):
        ...
        tk.Label(p3,text="Zorgt dat alle 4 kopieën up-to-date zijn.",
                 font=("Segoe UI",9),bg=PANEL,fg=FG).pack(anchor="w",pady=(0,6))
        self._btn(p3,"🔄  Synchroniseren",self._nasconfig_sync,bg=PANEL2,fg=FG).pack(anchor="w")
    ...
    def _pg_scripts(self):
        ...
        self._btn(br,"🔄  Vernieuwen",lambda:self._jump(15),bg=PANEL2,fg=FG).pack(side="left")
    ...
    def _pg_sysinfo(self, installatie_klaar=False):
        ...
        if not installatie_klaar:
            ...
            self._btn(br,"🔄  Vernieuwen",lambda:self._jump(17),bg=PANEL2,fg=FG).pack(side="left")
    ...
    def _pg_help(self):
        ...
        secties=[
            ...
            ("⚙️ NAS-software","Installeert Samba + Cockpit (netwerkschijf). Nextcloud installeer je apart via pinas_nextcloud.sh."),
            ("🌐 FileBrowser","Webbeheer voor bestanden via browser op http://[IP]:8080. Installeren, starten, stoppen."),
            ("🖥️ Desktop & VNC","Desktop (LXDE) en VNC installeren.\nPi 4 → RealVNC Viewer poort 5900.\nPi 5 → TigerVNC Viewer poort 5901. Start installer met: xhost +local:root && sudo python3 nas_installer.py"),
    ...
    def _pg_beheer(self):
        ...
        self._btn(br,f"✅  Ontbrekende aanmaken ({len(ontbreekt)})",
                  ...
                  bg=GREEN if ontbreekt else PANEL2,fg=BG if ontbreekt else FG,
                  bold=True).pack(side="left",padx=(0,8))
        self._btn(br,"➕  Eigen map/share",self._beheer_map_aanmaken,bg=PANEL2,fg=FG).pack(side="left",padx=(0,8))
        ...
        if os.path.exists("/home/pi/smart_plug_config.json"):
            ...
            self._btn(br3,"⏹  Uitzetten",self._seagate_uit,bg=PANEL2,fg=FG).pack(side="left")
            ...
            if wc_actief:
                self._btn(br4,"⏹  Stoppen",self._wc_stop,bg=PANEL2,fg=FG).pack(side="left",padx=(0,8))
    ...
    def _wc_install(self):
        ...
        def bg():
            # Het .service-bestand werd voorheen NIET aangemaakt, alleen
            # gekopieerd vanaf een pad dat moest al bestaan -- als dat
            # bestand er niet was, faalde de installatie stil en toonde
            # de UI toch altijd "OK", ongeacht de werkelijke uitkomst.
            if not os.path.exists("/home/pi/seagate_web.py"):
                self._beheer_log("FOUT: /home/pi/seagate_web.py ontbreekt. "
                                  "Upload dit bestand eerst (PiNAS map).","err")
                return
            svc = """[Unit]
...
WantedBy=multi-user.target
"""
            with open("/tmp/seagate-web.service","w") as f:
                f.write(svc)
            sh("sudo cp /tmp/seagate-web.service /etc/systemd/system/seagate-web.service")
            sh("sudo systemctl daemon-reload")
            sh("sudo systemctl enable --now seagate-web")
            import time; time.sleep(2)

            # Echte verificatie -- geen aangenomen succes meer.
            actief = sh("systemctl is-active seagate-web 2>/dev/null").strip() == "active"
            if actief:
                ip=get_ip()
                self.after(0,lambda:(
                ...
                foutdetail = sh("systemctl status seagate-web --no-pager -l 2>&1 | tail -n 10")
                self.after(0,lambda:(
                    self._beheer_log("FOUT: service is NIET actief na installatie.","err"),
                    self._beheer_log(foutdetail,"err")))
        import threading; threading.Thread(target=bg,daemon=True).start()
    ...
    def _pg_cockpit(self):
        ...
        self._btn(br2,"⏹  Stoppen",self._cockpit_stop,bg=PANEL2,fg=FG).pack(side="left",padx=(0,8))
        if ck_enabled:
            self._btn(br2,"Uitschakelen bij opstarten",self._cockpit_disable,bg=PANEL2,fg=FG).pack(side="left")
        else:
    ...
    def _pg_docker(self):
        """Docker installeren en beheren op de Pi 5."""
        self._head("🐳  Docker",FG)

        # Status ophalen
        docker_installed = self._run_cmd("which docker").strip() != ""
        docker_running   = self._run_cmd("sudo systemctl is-active docker 2>/dev/null").strip() == "active"
        docker_version   = self._run_cmd("docker --version 2>/dev/null").strip() if docker_installed else ""
        compose_ok       = self._run_cmd("docker compose version 2>/dev/null").strip() != ""

        # Status panel
        if docker_installed:
            status_color = "#1e3a2a" if docker_running else "#3a2a1e"
            status_text  = f"✅  Docker geïnstalleerd — {docker_version}"
            if not docker_running:
                status_text += "  (service gestopt)"
        else:
            status_color = "#3a2a1e"
            status_text  = "⚠️  Docker niet geïnstalleerd"

        p0 = self._panel(color=status_color)
        tk.Label(p0, text=status_color and status_text,
                 font=("Segoe UI",11,"bold"), bg=status_color,
                 fg=GREEN if docker_installed and docker_running else WARN).pack(anchor="w")
        if compose_ok:
            tk.Label(p0, text="✅  Docker Compose beschikbaar",
                     font=("Segoe UI",10), bg=status_color, fg=GREEN).pack(anchor="w")

        # Installeren / updaten
        p1 = self._panel()
        tk.Label(p1, text="Installatie", font=("Segoe UI",12,"bold"),
                 bg=PANEL, fg=ACCENT).pack(anchor="w", pady=(0,6))
        tk.Label(p1, text="Gebruikt de officiële Docker repository — correct voor Pi 5 (arm64).",
                 font=("Segoe UI",10), bg=PANEL, fg=FG).pack(anchor="w", pady=(0,8))
        ...
        def docker_install():
            ...
            self._run_cmds(cmds, done=lambda: self._jump(21))
        ...
        def docker_start():
            self._run_cmd("sudo systemctl start docker")
            self._jump(21)

        def docker_stop():
            self._run_cmd("sudo systemctl stop docker")
            self._jump(21)

        def docker_remove():
            if messagebox.askyesno("Docker verwijderen",
                "Docker en alle containers/images verwijderen?\nData in volumes blijft bewaard."):
                cmds = [
        ...
        br = tk.Frame(p1, bg=PANEL)
        br.pack(anchor="w", pady=(0,4))

        if not docker_installed:
            self._btn(br, "🐳  Docker installeren (Pi 5 methode)",
                      docker_install, bg=ACCENT, fg=BG, bold=True).pack(side="left")
        else:
            self._btn(br, "⬆  Docker updaten",
                      docker_update, bg=ACCENT, fg=BG, bold=True).pack(side="left", padx=(0,8))
            if docker_running:
                self._btn(br, "⏹  Stoppen",
                          docker_stop, bg=WARN, fg=BG).pack(side="left", padx=(0,8))
            else:
                self._btn(br, "▶  Starten",
                          docker_start, bg=GREEN, fg=BG).pack(side="left", padx=(0,8))
            self._btn(br, "🗑  Verwijderen",
                      docker_remove, bg="#7c1d1d", fg=FG).pack(side="left")

        # Docker Compose info
        p2 = self._panel()
        tk.Label(p2, text="Docker Compose", font=("Segoe UI",12,"bold"),
                 bg=PANEL, fg=ACCENT).pack(anchor="w", pady=(0,4))
        tk.Label(p2, text="Docker Compose is inbegrepen bij de installatie als plugin.\n"
                          "Gebruik: docker compose up -d",
                 font=("Segoe UI",10), bg=PANEL, fg=FG, justify="left").pack(anchor="w")

        # Draaiende containers
        if docker_installed and docker_running:
            containers = self._run_cmd("docker ps --format '{{.Names}}\t{{.Status}}\t{{.Image}}' 2>/dev/null").strip()
            p3 = self._panel()
            tk.Label(p3, text="Draaiende containers", font=("Segoe UI",12,"bold"),
                     bg=PANEL, fg=ACCENT).pack(anchor="w", pady=(0,4))
            if containers:
                for line in containers.split("\n"):
                    tk.Label(p3, text=f"  🟢 {line}",
                             font=("Segoe UI Mono",9), bg=PANEL, fg=FG).pack(anchor="w")
            else:
                tk.Label(p3, text="  Geen containers actief.",
                         font=("Segoe UI",10), bg=PANEL, fg=DIM).pack(anchor="w")
            br2 = tk.Frame(p3, bg=PANEL)
            br2.pack(anchor="w", pady=(8,0))
            self._btn(br2, "🔄  Vernieuwen", lambda: self._jump(21),
                      bg=PANEL2, fg=FG).pack(side="left")


if __name__=="__main__":
    if os.geteuid()!=0:
        # Automatisch herstarten met sudo
        import subprocess as _sp
        print("Herstarten met sudo...")
        _sp.execvp("sudo", ["sudo", "python3"] + sys.argv)
