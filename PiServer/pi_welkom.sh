...
pi_welkom_toon() {
    ...
    echo -e ""
    echo -e "  ${CYAN}${BOLD}3${R}  📊  Status & diagnose"
    echo -e "     ${DIM}Schijfgebruik, netwerk, services bekijken${R}"
    echo -e ""
    if [ "$updates" -gt 0 ]; then
        echo -e "  ${YELLOW}${BOLD}4${R}  ⬆  Scripts bijwerken vanuit SD-kaart"
        echo -e "     ${YELLOW}$updates bestand(en) in /boot/firmware/ zijn nieuwer${R}"
        echo -e ""
    else
        echo -e "  ${CYAN}${BOLD}4${R}  ⬆  Scripts bijwerken vanuit SD-kaart"
        echo -e "     ${DIM}Kopieer nieuwe scripts van /boot/firmware/ naar /home/pi/${R}"
        echo -e ""
    fi
    echo -e "  ${CYAN}${BOLD}5${R}  🚪  Pi afsluiten"
    echo -e "     ${DIM}Netjes afsluiten (wacht daarna 30 sec voor schijf uit)${R}"
    echo -e ""
    echo -e "  ${CYAN}${BOLD}6${R}  💻  Gewone terminal"
    echo -e "     ${DIM}Direct naar de commandoregel${R}"
    echo -e ""
    # Seagate status bepalen
    SEAGATE_STATUS=$(seagate_status)
    if [ "$SEAGATE_STATUS" = "aan" ]; then
        echo -e "  ${CYAN}${BOLD}7${R}  🔌  Seagate uitzetten  ${GREEN}(nu: aan)${R}"
    else
        echo -e "  ${CYAN}${BOLD}7${R}  🔌  Seagate aanzetten  ${DIM}(nu: uit)${R}"
    fi
    echo -e ""
    echo -e "  ${CYAN}${BOLD}8${R}  🔒  Uitloggen / sessie vergrendelen"
...
seagate_aan() {
    ...
    # De schijf moet opspinnen; udev mount hem daarna automatisch. Even geduld.
    echo -e "  ${CYAN}Wachten tot de schijf verschijnt en automatisch mount...${R}"
    ...
    if mountpoint -q "$MOUNT"; then
        ...
        echo -e "  ${YELLOW}!   Seagate aan, maar na 25s nog niet gemount. Controleer de schijf.${R}"
    fi
}
...
seagate_uit() {
    ...
    MOUNT=$(python3 -c "import json; c=json.load(open('$PLUG_CONFIG')); print(c.get('seagate_mount','/mnt/backup'))" 2>/dev/null || echo "/mnt/backup")
    # 1) Weiger als er ACTIEF geschreven wordt (open bestand RDWR/WRONLY).
    if sudo smbstatus --locks 2>/dev/null | grep -iE "RDWR|WRONLY" | grep -qv RDONLY; then
        echo -e "  ${YELLOW}!   Er wordt nu naar de backup geschreven.${R}"
        echo -e "  ${YELLOW}    Wacht tot dat klaar is en probeer opnieuw. Niets uitgezet.${R}"
        return 1
    fi
    # 2) Slapende/lees-sessies netjes sluiten zodat de unmount kan slagen.
    echo -e "  ${CYAN}Verbindingen naar de backup sluiten...${R}"
    sudo smbcontrol smbd close-share Backup 2>/dev/null
    sleep 2
    # 3) Schoon ontkoppelen terwijl de stroom er nog op staat.
    if mountpoint -q "$MOUNT"; then
        if ! sudo umount "$MOUNT" 2>/dev/null; then
            echo -e "  ${YELLOW}!   Kon $MOUNT niet ontkoppelen (nog in gebruik). Niets uitgezet.${R}"
            return 1
        fi
    fi
    # 4) Pas nu de plug uit.
    echo -e "  ${CYAN}Seagate uitzetten...${R}"
    python3 /home/pi/smart_plug.py uit 2>/dev/null
    echo -e "  ${GREEN}OK  Backup schoon ontkoppeld en Seagate uitgezet${R}"
}
