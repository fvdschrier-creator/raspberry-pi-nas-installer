...
def is_pi5():
    ...
    model = sh("cat /proc/device-tree/model 2>/dev/null || cat /sys/firmware/devicetree/base/model 2>/dev/null")
    return "Pi 5" in model or "Raspberry Pi 5" in model

def get_vnc_port():
    """Geef juiste VNC poort terug afhankelijk van Pi model."""
    ...
    if "no" in pw_auth.lower():
        ...
        if ask_yn("Wachtwoordlogin uitschakelen?","n"):
            ...
            if not keys or int(keys) == 0:
                ...
                # Bug 8 fix: cloud-init overschrijft sshd_config
                ...
                if os.path.exists(cloud_init):
                    ...
                    ok("cloud-init SSH instelling ook bijgewerkt")
...
def setup_nopasswd():
    ...
    sudoers.d-bestand (veiliger dan het hoofdbestand bewerken). Vereist
    voor de externe SSH-installatieknoppen in Pi NAS Menu, die anders
    blijven hangen op een wachtwoordprompt die via een niet-interactieve
    SSH-sessie nooit beantwoord kan worden. Veilig om herhaaldelijk te
    draaien."""
    try:
        user = sh("logname 2>/dev/null || echo pi") or "pi"
...
def update_rechten_service():
    ...
    # Wacht op de ECHTE mountpoints (max 60 sec) -- externe schijven via
    # smart plug kunnen pas seconden na boot daadwerkelijk gemount zijn,
    # waardoor deze service voorheen permanent op 'failed' kon blijven
    # staan voor een timing-probleem dat bij een volgende boot vanzelf
    # zou zijn opgelost.
    wacht_cmd = " && ".join(
...
def nas_config():
    ...
    # Nextcloud installeer je apart via pinas_nextcloud.sh - deze installer
    # doet alleen nog Samba + Cockpit.
    method="A"
    cfg={"method":method}
...
def desktop_menu():
    ...
    vnc_installed = bool(sh("which vncserver 2>/dev/null || dpkg -l realvnc-vnc-server 2>/dev/null | grep -c '^ii' || dpkg -l tigervnc-standalone-server 2>/dev/null | grep -c '^ii'"))
    vnc_active = sh("sudo systemctl is-active vncserver-x11-serviced 2>/dev/null || sudo systemctl is-active tigervnc 2>/dev/null || pgrep Xtigervnc > /dev/null && echo active").strip() == "active"
    ip = get_ip()
    ...
    if vnc_installed:
        ...
        ok(f"VNC is geïnstalleerd — {'actief op '+ip+':'+vnc_port if vnc_active else 'niet actief'}")
    else:
...
def deinstall_menu():
    ...
    if nc_ok:
        info("Nextcloud verwijderen: gebruik pinas_nextcloud_verwijderen.sh")
    print()
    ...
    if idx==6 or idx==-1: return

    if idx==5:
        warn("DIT VERWIJDERT: Samba, FileBrowser, Cockpit en Desktop.")
        warn("(Nextcloud: gebruik apart pinas_nextcloud_verwijderen.sh)")
...
def beheer_schijfruimte():
    ...
    print()
    # 8 augustus 2026: was een f-string met een backslash in de {}-expressie
    # (sh('...\"...')) - mag niet in Python < 3.12 (PEP 701 verruimt dit pas
    # vanaf 3.12), en de awk-aanhalingstekens waren daardoor ook niet goed
    # gebalanceerd (ontbrekende afsluitende "). Nu: shell-commando eerst in
    # een gewone variabele (geen backslash-beperking daarbuiten), en de
    # awk-string correct in enkele quotes gewrapt.
    _df_cmd = """df -h / | tail -1 | awk '{print $3"/"$2" gebruikt ("$5")"}'"""
    info(f"SD-kaart: {sh(_df_cmd)} ")
    pause()
...
def show_help():
    ...
    secties = [
        ...
        ("3  ⚙️   NAS-software",
         ["Samba + Cockpit — netwerkschijf voor Windows/iPhone/Android",
          "Nextcloud installeer je apart via pinas_nextcloud.sh"]),
        ("4  🌐  FileBrowser",
        ...
        ("5  🖥️   Desktop & VNC",
         ["Desktop installeren (LXDE + Tkinter) voor grafische installer",
          "Pi 4 → RealVNC · verbind via RealVNC Viewer op poort 5900",
          "Pi 5 → TigerVNC · verbind via TigerVNC Viewer op poort 5901",
          "           Download TigerVNC: https://github.com/TigerVNC/tigervnc/releases",
          "Pi 5: installer start met 'xhost +local:root && sudo python3 nas_installer.py'",
        ...
        ("7  🗑   Deïnstalleren",
         ["Verwijder Samba, FileBrowser, Cockpit of Desktop afzonderlijk",
          "Nextcloud verwijderen: apart via pinas_nextcloud_verwijderen.sh",
